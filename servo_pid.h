 ...
 * (c) 2016 Jeremy Simas Reeve (jsr@jsreeve.org)
 *
 ...
 */
void pid_clear_state(volatile pid_t *pid) {
  pid->i_component = 0;
  pid->old_error = LONG_MAX;
  pid->saturation = SATURATION_NONE;
}

int32_t pid_calculate_output(volatile pid_t *pid, int32_t target, int32_t actual) {
